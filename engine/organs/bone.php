...
class OrganBone{
	...
	private static function remove_ipsp_from_usable(&$usable_mem){
	...
	private static function remove_ipsp_from_usable_list($start,$end){
		...
		while (true){
			...
			if (ConstructionDlinkedListOpt::issetDlinkedListUnit($c_lp,'poly')){  
				...
				self::remove_ipsp_from_usable ($poly_result_array[$tmp]['usable'][$asm_id]['p']['mem_opt_able']);
				self::remove_ipsp_from_usable ($poly_result_array[$tmp]['usable'][$asm_id]['n']['mem_opt_able']);	   
			}elseif (ConstructionDlinkedListOpt::issetDlinkedListUnit($c_lp,'bone')){  
				$tmp = ConstructionDlinkedListOpt::getDlinkedList($c_lp,'bone');
				self::remove_ipsp_from_usable ($bone_result_array[$tmp]['usable'][$asm_id]['p']['mem_opt_able']);
				self::remove_ipsp_from_usable ($bone_result_array[$tmp]['usable'][$asm_id]['n']['mem_opt_able']);   
			}elseif (ConstructionDlinkedListOpt::issetDlinkedListUnit($c_lp,'meat')){  
				$tmp = ConstructionDlinkedListOpt::getDlinkedList($c_lp,'meat');
				self::remove_ipsp_from_usable ($meat_result_array[$tmp]['usable'][$asm_id]['p']['mem_opt_able']);
				self::remove_ipsp_from_usable ($meat_result_array[$tmp]['usable'][$asm_id]['n']['mem_opt_able']);
			}else{                                                    
				self::remove_ipsp_from_usable ($c_soul_usable[$asm_id]['p']['mem_opt_able']);
				self::remove_ipsp_from_usable ($c_soul_usable[$asm_id]['n']['mem_opt_able']);
			}
			...
			foreach ($c_bone_result_array['ipsp'] as $a => $b){
				...
				if (true === $b){   
					...
					if (isset($c_bone_result_array['usable'][$a]['p']['mem_opt_able'])){
						self::remove_ipsp_from_usable($c_bone_result_array['usable'][$a]['p']['mem_opt_able']);
					}			    
				}elseif (2 === $b){ 
					if (isset($c_bone_result_array['usable'][$a]['n']['mem_opt_able'])){
						self::remove_ipsp_from_usable($c_bone_result_array['usable'][$a]['n']['mem_opt_able']);
					}			    
				}
			}
		}
	...
	function init_bone_model(&$c_bone,$bone_obj){ 
		...
		self::inherit_bone_usable($bone_result_array[$bone_index],$soul_position);
	...
	public static function start($bone_obj,$language){		 
		...
		if ($z){
			...
			if (!self::collect_usable_bone_model ($bone_obj,$last_ipsp,$bone_model_repo[$z])){ 
